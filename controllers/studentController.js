  ...
  getAvailableAppointmentSlots: async (req, res) => {
    try {
      ...
      const bookedKeys = new Set(
        studentBookings
          .filter(b => b.teacherId) // Filter out bookings with null teacherId
          .map(b => `${b.teacherId.toString()}|${b.date.toISOString()}|${b.time}`)
      );

      // Filter out slots already booked by this student and slots with invalid teacher references
      const filteredSlots = availableSlots.filter(slot => {
        // Skip slots where teacherId is null or undefined
        if (!slot.teacherId || !slot.teacherId._id) {
          return false;
        }
